...
# 1. HP Coordinate Descent Sign Correction (Direction was reversed)
# 2. Initialization Strategy: Break symmetry using initial tau -> M-step
# 3. BIC df tolerance fixed to 1e-8
# 4. Adaptive Weights without normalization
#
# ADDITIONAL FIXES APPLIED (THIS SCRIPT):
# 5. BUGFIX: Mix-HP-AL adaptive block had typo: pi <- pil$pi  (pil undefined)
#           -> pi <- pil_fit$pi
# 6. min_cd propagation: avoid hard-coding min_cd=10 inside HP calls
#
# TRACE ADDED (YOU REQUESTED):
# - Progress (grid progress, rep/method progress)
# - Counts (grid index, EM iteration count)
# - Log-likelihood values (ll and delta-ll at EM trace points + final ll per candidate)
# ============================================================
  ...
  for (s in 1:n_start) {
    ...
    for (iter in 1:max_em) {
      ...
      ms <- mstep_mix_hp(
        ...
        max_cd = max_cd, tol_cd = tol_cd, min_cd = min_cd,   # [FIX 6] propagate min_cd
...
# - Progress: grid_id/grid_total (+ optional progress bar)
# - Counts: EM iterations (fit$em_iter)
# - Likelihood: final ll of candidate (fit$ll), and optional EM trace inside fit
# =========================
fit_by_bic <- function(y, X, method, m_set = 2:4,
                       ...
                       # ---- TRACE controls (defaults tuned for your request) ----
                       grid_trace_every = NULL,      # NULL -> auto
                       grid_print_best = TRUE,        # print when best BIC improves
                       grid_print_progress = TRUE,    # periodic progress line
                       em_verbose = FALSE,            # EM trace inside model fitting
                       em_trace_every = 10) {
...
# - prints rep/method progress + chosen model ll/bic/em_iter
# - calls fit_by_bic with grid progress + ll prints
# -------------------------
run_sim_corrected <- function(R = 20, n = 200, p = 60, snr = 50,
                              # grid progress + ll prints
                              show_grid_bar = TRUE,
                              grid_trace_every = NULL,
                              grid_print_best = TRUE,
                              grid_print_progress = TRUE,
                              # EM trace inside fit (optional, can be noisy)
                              em_verbose = FALSE,
                              em_trace_every = 10) {
...
# You said you want: 진행률/횟수/가능도값
# - show_grid_bar=TRUE : grid progress bar
# - grid_print_progress=TRUE : periodic [GRID-PROG] with best ll/bic updates
# - grid_print_best=TRUE : print when best BIC improves (includes ll, em_iter)
# - em_verbose=FALSE : keep EM-iteration logs off (turn TRUE if you want ll per EM step)
res <- run_sim_corrected(
  R = 5, n = 200, p = 60, snr = 50,
  show_grid_bar = TRUE,
  grid_trace_every = NULL,
  grid_print_best = TRUE,
  grid_print_progress = TRUE,
